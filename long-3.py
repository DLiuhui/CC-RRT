...
def main():
    ...
    obstacle_list = [
        (1.83, 74.57, 2.48, 1.13, np.deg2rad(90.0), 2.48 - 3.82/2, 1.13 - 1.59/2, 3.82/2, 1.59/2),
        # (5.36, 44.96, 3.96, 1.59, np.deg2rad(100.0),2.61, 1.19),
        (5.44, 65.03, 2.34, 1.11, np.deg2rad(89.0), 2.34 - 3.56/2, 1.11 - 1.54/2, 3.56/2, 1.54/2),
        (5.41, 94.42, 3.15, 1.37, np.deg2rad(90.0), 3.15 - 4.28/2, 1.37 - 1.65/2, 4.28/2, 1.65/2),
        # (9.0, 54.93, 4.08, 1.59, np.deg2rad(90.0), 2.6, 1.14),
        (9.124, 84.98, 2.63, 1.23, np.deg2rad(90.0), 2.63 - 3.93/2, 1.23 - 1.66/2, 3.93/2, 1.66/2),
        # (12.73, 44.84, 3.4, 1.5, np.deg2rad(89.90), 2.28, 1.13),
        # (12.55, 64.96, 4.45, 1.71, np.deg2rad(90.0), 2.83, 1.18),
    ]
    ...
    area = [0, 15, 60, 100]  # x-min x-max y-min y-max
    cc_rrt = CLRRT(
        car=car,
        start=start,
        goal=goal,
