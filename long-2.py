...
def main():
    ...
    obstacle_list = [
        (1.978, 74.55, 2.67, 1.2, np.deg2rad(90.0), 2.67 - 4.13/2, 1.2 - 1.63/2, 4.13/2, 1.63/2),
        # (5.36, 44.96, 3.96, 1.59, np.deg2rad(100.0),2.61, 1.19),
        (5.47, 64.54, 2.47, 1.14, np.deg2rad(89.0), 2.47 - 3.79/2, 1.14 - 1.63/2, 3.79/2, 1.63/2),
        #   (5.5, 95, 5.36, 2.03, np.deg2rad(90.0),5.36/2, 2.03/2),
        (9.0, 54.93, 2.6, 1.14, np.deg2rad(90.0), 2.6 - 4.08/2, 1.14 - 1.59/2, 4.08/2, 1.59/2),
        (9.02, 84.86, 2.65, 1.32, np.deg2rad(90.0), 2.65 - 4.37/2, 1.32 - 1.67/2, 4.37/2, 1.67/2),
        # (12.73, 44.84, 3.4, 1.5, np.deg2rad(89.90), 2.28, 1.13),
        (12.55, 64.96, 2.83, 1.18, np.deg2rad(90.0), 2.83 - 4.45/2, 1.18 - 1.71/2, 4.45/2, 1.71/2),
    ]
    ...
    area = [0, 15, 48, 100]  # x-min x-max y-min y-max
    cc_rrt = CLRRT(
        car=car,
        start=start,
        goal=goal,
