...
class CCRRT:
    ...
    def __init__(self, car, start, goal, obstacle_list, rand_area):
        ...
        # params
        self.k_cc = 1000 # for chance_constrain value: node_cost = time + k_cc * node.cc
        ...
        self.p_safe = 0.95 # p_safe for chance constraint
    ...
    def get_chance_constrain(self, current):
        ...
        for obs in self.obstacle_list:
            ...
            for a, b in zip(A, B):
                ...
                abs_mat = np.diag([obs[6]*math.sin(angle) + obs[5]*math.cos(angle), obs[5]*math.sin(angle) + obs[6]*math.cos(angle), obs[4]])
    ...
    def is_feasible(self, node):
        ...
        if node.x < -7.0:
            ...
            if node.y > 16.0 or node.y < -7.0:
                valid = False
            elif node.y <= 16.0 and node.y >= -1.75:
                valid = True
            elif node.y < -1.75 and node.x > -1.75:
                valid = True
            elif node.x + node.y + 8.75 > 0.0:
                valid = True
        elif node.x < 7.0:
            if node.y < -7.0:
                valid = False
                ...
                valid = True
            elif node.y + node.x - 7.0 < 0.0:
                valid = True
        elif node.x < 21.0 and node.y < 0.0 and node.y > -7.0:
            valid = True
        # 碰撞检测
